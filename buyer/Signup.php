...
include "../partials/db.php";
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if($username_check <= 0){
            $exists = "SELECT * FROM `users` WHERE u_email ='$email' ";
            $result_email = mysqli_query( $conn,$exists);
            $email_check= mysqli_num_rows($result_email);

            if($email_check <= 0){
                if($password == $cpassword && $password != ""){
                    $sql = "INSERT INTO `users` (`user_id`, `username`, `name`, `u_email`, `u_password`, `status`) VALUES (NULL,'$username', '$name', '$email', '$password', '1')";
...
<div class="containers">
    <form action="Signup.php" method="POST" autocomplete="on">
        ...
        <div class="login">
            <input placeholder="Email Address" type="email" class="email" name="email" required>
        </div>
        <div class="login">
        ...
        <div class="signin">
            <a href="login_1.php">Already a member ? Sign in</a>
        </div>
    </form>
</div>
<script>
